...
<main>
    <section class="hero">
        ...
        <div class="hero-overlay">
            ...
            <p class="font-body-lg hero-description">Elektronika, żywność, materiały budowlane i narzędzia — ceny dopasowane do Twojego cennika B2B.</p>
        ...
        <div class="products-grid">
            ...
            <div class="product-card" onclick="location.href='<?= e(appUrl('/public/product.php?id=' . $product['id'])) ?>'">
                ...
                <div class="product-info">
                    ...
                    <p class="font-label-sm product-card-info-meta"><?= e(typeLabel($product['type'])) ?> · Stan: <?= e((string) $stock) ?></p>
                    <?php if ($price !== null): ?>
                        <?php if ($isBusiness): ?>
                            <p class="product-price product-card-price-display"><?= e(formatMoney($price)) ?> <small class="product-price-small-label">netto</small></p>
                        <?php else: ?>
                            <p class="product-price product-card-price-display"><?= e(formatMoney(grossFromNet($price))) ?> <small class="product-price-small-label">brutto</small></p>
                        <?php endif; ?>
                    <?php else: ?>
                    <p class="product-price product-card-no-price">Brak ceny</p>
                    <?php endif; ?>
                    <?php if ($price !== null && $stock > 0): ?>
                    <form method="post" action="<?= e(appUrl('/public/cart_action.php')) ?>" onclick="event.stopPropagation();" class="product-card-add-to-cart-form">
                        <?= csrfField() ?>
                        <input type="hidden" name="action" value="add">
                        <input type="hidden" name="product_id" value="<?= (int) $product['id'] ?>">
                        <input type="hidden" name="quantity" value="1">
                        <button type="submit" class="product-button product-card-add-to-cart-button">Dodaj do koszyka</button>
                    </form>
                    <?php endif; ?>
                </div>
            </div>
